...
### Belangrijke elementen bij het kiezen van database

- Inspanning om het te leren: heeft de ontwikkelaar al kennis van de werking van de database of nog niet?
- Samenwerking: zijn er libraries en drivers beschikbaar van de database voor de gekozen programmeertaal?
- Prestatie: voor de meeste projecten niet het belangrijkst aangezien bijna alle veel gebruikte databases snel genoeg zijn voor de meeste toepassingen.
...
Voor de database gaat er in dit project gebruik gemaakt worden vna MongoDB. Dit omdat het een grote community heeft, een goede intergratie met .NET Core en een Key-Document structuur database omgeving is. Daarnaast is er weinig inspanning nodig om deze techniek te leren en is de prestatie goed genoeg.
